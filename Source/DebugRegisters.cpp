...
void CDebugRegisters::Export(void)
{
  enum_<CDebugRegisters::RegisterSource>("RegisterSource")
    .value("DEBUGGEE", CDebugRegisters::DEBUGGEE)
    .value("EXPLICIT", CDebugRegisters::EXPLICIT)
    .value("FRAME", CDebugRegisters::FRAME)
    ;
...
const dict CDebugRegisters::GetRegisters(void) const
{
  ...
  for (size_t i=0; i<number; i++)
  {
    ...
    CDebugRegister reg(this, i, std::string(name, size-1), (ValueType) desc.Type);
