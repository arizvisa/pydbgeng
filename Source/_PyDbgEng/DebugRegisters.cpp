...
{
	enum_<RegisterSource>("RegisterSource")
		.value("DEBUGGEE", RegisterSource::DEBUGGEE)
		.value("EXPLICIT", RegisterSource::EXPLICIT)
		.value("FRAME",    RegisterSource::FRAME)
	;
	...
	class_<CDebugRegister>("DebugRegister", no_init)
		.add_property("Index", &CDebugRegister::GetIndex)
		.add_property("Name", &CDebugRegister::GetName)
		.add_property("Type", &CDebugRegister::GetType)
		.add_property("Value", &CDebugRegister::GetValue, &CDebugRegister::SetValue)
		.def(int_(self))
		.def(float_(self))
	;

	implicitly_convertible<CDebugRegister, int>();
	implicitly_convertible<CDebugRegister, double>();

	class_<CPseudoRegister>("PseudoRegister", no_init)
		.add_property("Index", &CPseudoRegister::GetIndex)
		.add_property("Name", &CPseudoRegister::GetName)
		.add_property("TypeModule", &CPseudoRegister::GetTypeModule)
		.add_property("TypeId", &CPseudoRegister::GetTypeId)
		.add_property("Value", &CPseudoRegister::GetValue, &CDebugRegister::SetValue)
		.def(int_(self))
		.def(float_(self))
	;

	implicitly_convertible<CPseudoRegister, int>();
	implicitly_convertible<CPseudoRegister, double>();
}
